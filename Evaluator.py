...
class SimplifiedEvaluator:  
    def __init__(self, player, usePromotion = True, useTrade = True, usePST = True):
        ...
        # heuristic has three components. These three variables control which ones are active
        # set all 3 to true for max strength
    ...
    def getMoveScore(self, board, move, endgame):
        """
        Base score: total value of player's board minus that of the opponent
        1. Is there a promotion?
        2. capturing a piece?
            a. punish capturing a weak piece with a strong piece
            b. reward capturing a strong piece with a weak one
        3. How has the position changed?
        """
